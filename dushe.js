...
(function () {
  ...
  // 把截获到的地址推给 App 原生播放器(纯 headers 安全版: 不做任何 m3u8 改写/data URI, 永不崩)
  function pushPlay(url, auto) {
    url = absUrl(url);
    if (!url || url === lastPushed) return;
    // 防崩保险: 地址异常超长(>8KB)不推, 避免 Android Intent 传输超限崩溃
    ...
    whenFm().then(function (fm) {
      try {
        // 裸播: headers / credentials 都不传 —— 实证传 headers 也会触发本地网关(127.0.0.1:9978/webResource),
        // 而网关取流先发 HEAD, 部分 CDN(如 yzzy31)HEAD 返回 502 → 网关失败 → 转圈。
        // 不传任何请求头, ExoPlayer 直连 GET(毒舌 CDN 都开放, 任意UA/无referer 都 200)。
        var opts = {};
        var pic = pagePoster();
        if (pic) { opts.pic = pic; opts.wallPic = pic; }
        fm.play(url, pageTitle(), opts);
        toast(auto ? "已用App播放" : "正在播放");
        log("play(raw) ->", pageTitle(), "| pic", pic ? "yes" : "no", "|", url);
  ...
  function ensureButton() {
    if (!isPlayPage()) {
      var ex = document.getElementById("fm-dushe-btn"); if (ex) ex.remove();
      var ef = document.getElementById("fm-dushe-fs"); if (ef) ef.remove();
      return;
    }
